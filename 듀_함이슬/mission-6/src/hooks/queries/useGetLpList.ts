...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
    return useQuery({
        ...
        //데이터가 신선하다고 간주하는 시간
        //이 시간동안은 캐시된 데이터를 그대로 사용. 컴포넌트가 마운트 되거나 창에 포커스 들어오는 경우도 재요청X
        ...
        // 기본 값은 3회 정도, 네트워크 오류 등 임시적인 문제를 보완할 수 있음
        // retry: 3,

        // initialData: 쿼리 실행 전 미리 제공할 초기 데이터를 설정함
        // 컴포넌트가 렌더링 될 때 빈 데이터 구조를 미리 제공해 로딩 전에도 안전하게 UI 구성할 수 있게 해줌

        // 파라미터가 변경될 때 이전 데이터를 유지하여 UI 깜빡임을 줄여줌
        // ex: 페이지네이션시 페이지 전환 사이에 이전 데이터를 보여주어 사용자 경험 향상시킴
