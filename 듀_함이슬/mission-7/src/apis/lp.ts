...
import type { RequestLPDto, ResponseLPListDto, ResponseLPDto, ResponseLikeLpDto, LpCreateResponseDto, CreateLpsDto, UpdateLpsDto, LpUpdateResponseDto, LpDelteResoponseDto } from "../types/lp";
...
export const deleteLp = async (lpid: number,): Promise<LpDelteResoponseDto> => {
    const { data } = await axiosInstance.delete<LpDelteResoponseDto>(
        `/v1/lps/${lpid}`
    );
    return data;
};